...
class Server(Connector):
    ...
    def __init__(self, ip='localhost', port=8080):
        ...
        if not os.path.exists('.\\Server'):
            os.mkdir('.\\Server')
        if not os.path.exists('.\\Server\\cache'):
            os.mkdir('.\\Server\\cache')
        if not os.path.exists('.\\Server\\cache\\users\\'):
            os.mkdir('.\\Server\\cache\\users\\')
        if not os.path.exists('.\\Server\\cache\\server\\'):
            os.mkdir('.\\Server\\cache\\server\\')
            ...
            if headers['type'] == "message":
                ...
                while t != 0:
                    if t < 1024:
                        ...
                        t -= 1024
                headers = {"code":0,"type":"current users","content-type": "list","cu":list(self.current_users.keys())+["%all%"]}
                headers = json.dumps(headers,ensure_ascii=False).encode('utf-8-sig')
                headers_len = struct.pack('i',len(headers))
                for i in list(self.current_users.values()):
                        i.send(headers_len)
